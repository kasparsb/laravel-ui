...
(function(){
    ...
    var componentBaseUrl = @json(asset('/vendor/ui/dist/components/'));
    var packageVersion = @json($ui_package_version);
    var cacheBuster = @json($ui_cache_buster);
    var svgsUrl = @json(route('ui::svgs'));
    ...
    function collectComponentNames(root) {
        ...
        else if (roots instanceof Element || roots instanceof Document || roots instanceof DocumentFragment) {
            roots = [roots];
        }
        else {
        ...
        roots.forEach(function(rootEl) {
            ...
            if (rootEl.matches && rootEl.matches('[data-ui-component-scripts]')) {
                els.push(rootEl);
            }

            if (rootEl.querySelectorAll) {
                els = els.concat(Array.from(rootEl.querySelectorAll('[data-ui-component-scripts]')));
            }

            els.forEach(function(el) {
                el.dataset.uiComponentScripts.split(/\s+/).forEach(function(name) {
    ...
    function collectSvgIconNames(root) {
        ...
        roots.forEach(function(rootEl) {
            ...
            if (rootEl.matches && rootEl.matches('[data-ui-svg-icons]')) {
                els.push(rootEl);
            }

            if (rootEl.querySelectorAll) {
                els = els.concat(Array.from(rootEl.querySelectorAll('[data-ui-svg-icons]')));
            }

            els.forEach(function(el) {
                el.dataset.uiSvgIcons.split(/\s+/).forEach(function(name) {
                    if (name && names.indexOf(name) < 0) {
                        names.push(name);
                    }
                });
